...
message () {
	local stat="$1"
	echo "TCP status is ${stat} - ${info}|Total=${total_connections_int};${warning};${critical};${min};${max};TIME_WAIT=${timewait_connections_int};;CLOSE_WAIT=${closewait_connections_int};;ESTABLISHED=${established_connections_int};;SYN_RECV=${synrecv_connections_int};;LAST_ACK=${lastack_connections_int};;"
}
...
stat_count(){
    local stat_str=$1
    str=`echo "${info}"|grep -oP "$stat_str:\d+"|awk -F':' '{print $2}'`
    if [ ! $str ];then
        str="0"
    fi
    int=`echo "${str}*1"|bc`
...
timewait_connections_int=`stat_count "TIME_WAIT"`
closewait_connections_int=`stat_count "CLOSE_WAIT"`
established_connections_int=`stat_count "ESTABLISHED"`
synrecv_connections_int=`stat_count "SYN_RECV"`
lastack_connections_int=`stat_count "LAST_ACK"`
